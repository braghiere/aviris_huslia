...
# Configuration
# TEST MODE: 10x10 area
const TEST_MODE = true
const TEST_ROWS = 10
const TEST_COLS = 10
const CHECKPOINT_FILE = "data/test_checkpoint_10x10_eki_v4.json"
const OUTPUT_FILE = "data/test_output_10x10_eki_v4.nc"
const BATCH_SIZE = 10  # Process 10 rows per batch for testing
...
# TEST MODE: Use 80 workers for 10x10 test
num_workers = min(80, Sys.CPU_THREADS ÷ 4)
addprocs(num_workers; exeflags="--project")
println("✅ Using $num_workers parallel workers for 10x10 TEST (v4)")
...
@everywhere begin
    ...
    # Cache spectral bounds (computed once per worker)
    ...
    # Forward model - V4 with meso_n and CI support
    function target_curve(wls::Vector{FT}, params::Dict{String,FT})::Vector{FT}
        SHIFT = deepcopy(SHIFT_BAK)
        
        # Update leaf traits (optimized with get() for single lookup)
        ...
        for leaf in SHIFT.plant.leaves
            # Fixed meso_n for boreal needleleaf
            if !isnan(meso_n_val)
        ...
        # LAI with clumping index
        if haskey(params, "lai")
            lai_val = params["lai"]
            if haskey(params, "ci") && !isnan(params["ci"])
                lai_val = lai_val * params["ci"]  # Effective LAI = LAI × CI
            end
        ...
        # Extract reflectance (use cached bounds, @inbounds for speed)
        tar_ys = similar(wls)
...
@everywhere function compute_traits_and_gpp(params, weather_df, spac_ref, config, saving_dict, lai_lidar, ci_lidar)
    ...
    # Compute GPP only if traits are valid
    gpp = NaN
...
# TEST MODE: Extract 10x10 area
global test_offset_lat = 0
global test_offset_lon = 0
if TEST_MODE
    println("\n🧪 TEST MODE: Extracting $TEST_ROWS x $TEST_COLS area")
    test_offset_lat = div(full_lat_size, 2) - div(TEST_ROWS, 2)
    test_offset_lon = div(full_lon_size, 2) - div(TEST_COLS, 2)
    reflectance_3d = reflectance_3d[test_offset_lon:(test_offset_lon+TEST_COLS-1), test_offset_lat:(test_offset_lat+TEST_ROWS-1), :]
    lat_values = lat_values[test_offset_lat:(test_offset_lat+TEST_ROWS-1)]
    lon_values = lon_values[test_offset_lon:(test_offset_lon+TEST_COLS-1)]
    println("   Offset: lat=$(test_offset_lat), lon=$(test_offset_lon)")
end
...
for batch_start in start_row:BATCH_SIZE:lat_size
    ...
    for i in batch_start:batch_end
        for j in 1:lon_size
            ...
            # V4: Get LiDAR values using GLOBAL indices
            global_lat_idx = test_offset_lat + i
            global_lon_idx = test_offset_lon + j
            lai_obs = get_lidar_value(global_lat_idx, global_lon_idx, lai_map)
            ci_obs = get_lidar_value(global_lat_idx, global_lon_idx, ci_map)
...
println("\n" * "="^80)
if TEST_MODE
    println("✅ 10x10 TEST COMPLETE! (EKI v4)")
else
...
println("\n🎉 All done! EKI v4 processing complete.")
println("   Key changes from v3:")
println("   ✅ LAI from LiDAR (not optimized)")
println("   ✅ CI from LiDAR (not optimized)")
println("   ✅ Only 4 optimized parameters: cab, lwc, cbc, pro")
println("   ✅ Soil moisture = 0.70 (near saturation)")
println("   ✅ 80 workers for parallel processing")

...
num_workers = min(24, Sys.CPU_THREADS ÷ 2)  # Use half of CPU threads (max 24)
if length(workers()) == 1
...
function process_batch!(datafile::String, batch_i::Int, batch_j::Int, batch_size::Int, batch_dir::String)
    ...
    batch_file = "$batch_dir/batch_$(batch_i)_$(batch_j).nc"
    
    # Skip batch if already computed
    ...
    if isempty(params)
        @info "No valid pixels in batch $(batch_i)-$(batch_j), skipping..."
        return
    end

    @info "Processing batch $(batch_i)-$(batch_j) with $(length(params)) valid pixels."
...
function merge_batches!(batch_dir::String, final_output::String)
    ...
    for batch_file in batch_files
        ...
        for trait in trait_names
            ...
            trait_matrices[trait][batch_i:batch_i+size(batch_data,1)-1, batch_j:batch_j+size(batch_data,2)-1] .= batch_data
        end
    end
...
process_all_batches!(datafile, 50, batch_dir)
